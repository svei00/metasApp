...
function Details() {
  ...
  const onChange = (event, prop) => {
    // onChange event (This is very important)
    setForm((state) => ({ ...state, [prop]: event.target.value }));
    console.log(form);
  };

  useEffect(() => {
    // Now it wors with useParams so when chance the id it changes the whole content
    // console.log(form);
  ...
  return (
    <div className="card">
      <form className="p-4">
        ...
        <label className="label">
          How many times have you complethed this?
          <input
            className="input"
            type="number"
            value={complete}
      ...
      <div className={styles.buttons}>
        {!id && (
          <button // We use !id to determine which button to show
        ...
        {id && (
          <button // We use id to determine if show update or create button
